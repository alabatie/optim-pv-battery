...
class Simulation(object):
    ...
    def simulate_timestep(self, battery_controller, current_time, timestep):
        ...
        # set handy variables
        sell_price = timestep.price_sell_00
        buy_price = timestep.price_buy_00

        # buy or sell energy depending on needs
        price = buy_price if grid_energy >= 0 else sell_price
        price_without_battery = buy_price if grid_energy_without_battery >= 0 else sell_price

        # compute spending for timestep
        money_spent_at_t = grid_energy * (price / 1000.)
        money_spent_without_battery_at_t = grid_energy_without_battery * (price_without_battery / 1000.)

        # calculate spending based on price per kWh and energy per Wh
        self.money_spent += money_spent_at_t
        self.money_spent_without_battery += money_spent_without_battery_at_t
        self.score = (self.money_spent - self.money_spent_without_battery) / np.abs(self.money_spent_without_battery)
        ...
        # compute reward at t
        money_saved = money_spent_without_battery_at_t - money_spent_at_t

        return money_saved, grid_energy

    def simulate_battery_charge(self, initial_state_of_charge, proposed_state_of_charge,
...
if __name__ == '__main__':
    ...
    output_dir = simulation_dir/'output/policy'
    ...
    for site_id, parameters in metadata.iterrows():
        ...
        if site_data_path.exists():
            ...
            for batt_id in [1, 2]:
                ...
                for g_id, g_df in site_data.groupby('period_id'):
                    ...
                    results.append({
                        ...
                        'score': (money_spent - money_no_batt) / abs(money_no_batt),
                    })
